...
export default function EventDetailsVisualSplit() {
  ...
  const border = isDark ? "border-white/[0.06]" : "border-[#F0B100]/30";
  const innerSection = isDark ? "bg-[#11122b]" : "bg-gray-100";

  if (loading || !event) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${bg}`}>
        <div className="w-10 h-10 border-4 border-[#F0B100] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div
      className={`min-h-screen relative overflow-hidden px-4 sm:px-6 md:px-12 lg:px-20 xl:px-28 py-32 transition-colors duration-300 ${bg} ${text}`}
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div
          className={`grid grid-cols-1 lg:grid-cols-12 gap-0 rounded-3xl overflow-hidden border ${border} ${cardBg} shadow-2xl items-stretch`}
        >
          <div className="lg:col-span-5 relative h-[350px] lg:h-auto min-h-[400px] bg-neutral-900">
            {!imageError && event.image_url ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-gray-500 text-xs font-black uppercase tracking-widest">
                {t("No Preview Available") || "No Preview Available"}
              </div>
            )}
          ...
          <div className="lg:col-span-7 p-6 sm:p-10 md:p-12 flex flex-col justify-between">
            ...
            >
              <div className="space-y-4">
                <span className="inline-flex items-center px-3 py-1 text-[10px] font-black tracking-widest uppercase font-mono rounded-full bg-[#F0B100]/10 text-[#F0B100] border border-[#F0B100]/30">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#F0B100] animate-pulse mr-1.5 ml-1.5" />
                  {event.status}
                </span>
                ...
                <div className="relative">
                  ...
                  <div className="w-16 h-[4px] bg-[#F0B100] mt-3 rounded-full" />
                </div>
              </div>

              <div className="flex flex-wrap gap-3">
                <div
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${border} ${innerSection} text-xs font-bold`}
                >
                  <CalendarDays size={14} className="text-[#F0B100]" />
                ...
                <div
                  className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${border} ${innerSection} text-xs font-bold`}
                >
                  <MapPin size={14} className="text-[#F0B100]" />
                  <span className="text-gray-400">{event.location}</span>
                </div>
              </div>

              <div
                className={`flex-1 flex flex-col p-5 rounded-2xl border ${border} ${innerSection} min-h-[250px]`}
              >
                ...
                <div className="max-h-[280px] lg:max-h-[340px] overflow-y-auto pr-2 custom-modern-scroll flex-1">
                  <p
                    className={`text-sm sm:text-base leading-relaxed whitespace-pre-line font-medium ${
