...
import { UPDATE_MY_AGENT_LEGAL_STATUS } from "@/app/graphql/mutation/legal-verification.mutations";

interface AgentLegalData {
  id: string;
  is_fifa_certified: boolean;
  fifa_license_id?: string | null;
}

export default function AgentLegalStatusPage() {
  ...
  const [isFifaCertified, setIsFifaCertified] = useState<boolean>(false);
  const [fifaLicenseId, setFifaLicenseId] = useState<string>("");

  const fetchStatus = useCallback(async () => {
    try {
      const result = await fetchGraphQL<{
        myAgentLegalStatus: AgentLegalData | null;
      }>(GET_MY_AGENT_LEGAL_STATUS);
      if (result.data?.myAgentLegalStatus) {
        const info = result.data.myAgentLegalStatus;
        setIsFifaCertified(!!info.is_fifa_certified);
        setFifaLicenseId(info.fifa_license_id || "");
      }
    } catch (err) {
      console.error("Error fetching agent legal status:", err);
    } finally {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (isFifaCertified && !fifaLicenseId.trim()) {
      toast.error(t("Please enter your FIFA License ID"));
      return;
    }
    ...
    try {
      const result = await fetchGraphQL(UPDATE_MY_AGENT_LEGAL_STATUS, {
        input: {
          is_fifa_certified: isFifaCertified,
          fifa_license_id: isFifaCertified ? fifaLicenseId.trim() : null,
        },
      });
      ...
      console.error("Error updating agent legal status:", err);
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="flex justify-center items-center gap-6 mb-10">
          {/* User Profile Icon - يروح للبروفايل */}
          <button
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              ...
              <label className="flex items-start gap-4 cursor-pointer p-4 rounded-lg hover:bg-yellow-400/5 transition">
                <input
                  type="checkbox"
                  checked={isFifaCertified}
                  onChange={(e) => setIsFifaCertified(e.target.checked)}
              ...
              {isFifaCertified && (
                <div className="mt-6 pt-4 border-t ${isDark ? 'border-[#1e2d5a]' : 'border-gray-200'}">
                  ...
                  >
                    🆔 {t("fifaLicenseIdLabel")} *
                  </label>
                  <input
                    type="text"
                    value={fifaLicenseId}
                    onChange={(e) => setFifaLicenseId(e.target.value)}
                    placeholder={t("fifaLicenseIdPlaceholder")}
                    required={isFifaCertified}
