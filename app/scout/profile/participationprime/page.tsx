...
export default function ScoutPackagesPage() {
  ...
  const [packages, setPackages] = useState<ScoutPackageType[]>([]);
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        ...
        const res: any = await fetchGraphQL(GET_SCOUT_PACKAGES);

        console.log("SCOUT PACKAGES:", res);
  ...
  // SELECT PACKAGE
  // =========================
  const handleSelectPackage = (item: ScoutPackageType) => {
    const realEnum =
      item.package_type === "Scout Premium"
        ? "SCOUT_PREMIUM"
        : item.package_type === "Scout Basic"
        ? "SCOUT_BASIC"
        : item.package_type === "كشاف محترف"
        ? "SCOUT_PREMIUM"
        : item.package_type === "كشاف مبتدئ"
        ? "SCOUT_BASIC"
        : item.package_type;

    console.log("SCOUT ENUM:", realEnum);
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-20">
          ...
          <h1 className="text-5xl font-black">{t("Organization Packages")}</h1>
        </div>
  ...
  return (
    <div
      className={`
        ...
        ${index === 1 ? "z-20 scale-105 -translate-y-5" : "z-10 hover:-translate-y-3"}
      `}
    >
      {/* BORDER */}
      ...
      >
        ...
        <div className="mt-8 space-y-3">
          ...
          <Feature
            icon={<Search size={18} />}
            text={`${t("Unlimited Requests")}: ${
              item.can_send_unlimited ? t("Available") : t("Unavailable")
            }`}
          />

          <Feature
            icon={<Shield size={18} />}
            text={`${t("Special Players Access")}: ${
              item.can_access_special_players ? t("Available") : t("Unavailable")
            }`}
          />
          ...
          <Feature
            icon={<Crown size={18} />}
            text={`${t("Priority Listing")}: ${
              item.priority_listing ? t("Available") : t("Unavailable")
            }`}
          />
        </div>
